...
function buildSafeReconcilePlan() {
  ...
  var data = waspSheet.getRange(2, 1, lastRow - 1, 20).getValues();
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var dateCode = reconcileFormatDate_(row[8]);
    var katanaQty = reconcileToNumber_(row[10]);
    var waspQty = reconcileToNumber_(row[12]);
    var match = String(row[15] || '').trim().toUpperCase();
    var key = sku + '|' + site;
...
function getReconcilePendingWarnings_(ss) {
  ...
  if (waspSheet && waspSheet.getLastRow() > 1) {
    var waspData = waspSheet.getRange(2, 1, waspSheet.getLastRow() - 1, 17).getValues();
    for (var i = 0; i < waspData.length; i++) {
      var sku = String(waspData[i][0] || '').trim();
      if (!sku || sku.indexOf(' > ') >= 0) continue;
      var status = String(waspData[i][16] || '').trim().toUpperCase();
