 ...
 */
function sheetDateValue_(value) {
...
function sheetNumberValue_(value) {
  return (value instanceof Date) ? 0 : (parseFloat(value) || 0);
}
...
function pushToWasp() {
  ...
  // Read all data rows (A2:S{lastRow})
  ...
  // Find rows with Status === 'Pending' (col Q, index 16)
  var deleteRows = [];
  for (i = 0; i < data.length; i++) {
    var status = String(data[i][16]).trim();
    if (status === 'Pending') {
      pendingRows.push(i);
  ...
  for (j = 0; j < pendingRows.length; j++) {
    ...
    if (actions.length === 0) {
      ...
      data[rowIndex][16] = 'Synced ' + formatTimestamp();
      backgrounds[rowIndex][16] = '#ffffff';
      successCount++;
      continue;
    }
    ...
    if (allSuccess) {
      ...
      // so mutations below (e.g. data[rowIndex][11] = ...) would corrupt log reads.
      var logSku = String(row[0]).trim();
      var logSite = String(row[4]).trim();
      var logLoc = String(row[5]).trim();
      var logLot = String(row[7]).trim();
      var logExpiry = (row[8] instanceof Date) ? Utilities.formatDate(row[8], 'America/Vancouver', 'yyyy-MM-dd') : String(row[8] || '').trim();
      var logOrigQty = (row[11] instanceof Date) ? 0 : (parseFloat(row[11]) || 0);
      var logNewQty = (row[12] instanceof Date) ? 0 : (parseFloat(row[12]) || 0);
      ...
      data[rowIndex][11] = data[rowIndex][12];  // Orig WASP Qty = W.Qty
      data[rowIndex][13] = data[rowIndex][4];   // Orig Site = Site
      data[rowIndex][14] = data[rowIndex][5];   // Orig Location = Location
      data[rowIndex][17] = data[rowIndex][7];   // Orig Lot = Lot
      data[rowIndex][18] = data[rowIndex][8];   // Orig DateCode = DateCode
  ...
  if (Object.keys(deleteSkus).length > 0) {
    ...
    for (var di2 = 0; di2 < data.length; di2++) {
      ...
      var dOrigQty = sheetNumberValue_(dRow[11]);
      var dOrigSite = String(dRow[13] || '').trim();
      var dOrigLocation = String(dRow[14] || '').trim();
      var dOrigLot = String(dRow[17] || '').trim();
      var dOrigDateCode = sheetDateValue_(dRow[18]);
      var dHasOrig = !!(dOrigSite || dOrigLocation || dOrigLot || dOrigDateCode || Math.abs(dOrigQty) > 0.01);

      var dSite     = dHasOrig ? dOrigSite : String(dRow[4] || '').trim();
      var dLocation = dHasOrig ? dOrigLocation : String(dRow[5] || '').trim();
      var dLot      = dHasOrig ? dOrigLot : String(dRow[7] || '').trim();
      var dDateCode = dHasOrig ? dOrigDateCode : sheetDateValue_(dRow[8]);
      var dWQty = dHasOrig ? dOrigQty : sheetNumberValue_(dRow[12]);
 ...
 * 19-col Wasp tab: A(0)SKU B(1)Name C(2)Cost D(3)Type E(4)Site F(5)Location
 *   G(6)LotTracked H(7)Lot I(8)DateCode J(9)UOM K(10)K.Qty L(11)OrigWASPQty
 *   M(12)W.Qty N(13)OrigSite O(14)OrigLocation P(15)Match Q(16)Status
 *   R(17)OrigLot S(18)OrigDateCode
 *
 * Two cases:
 *   Case 1: New row (no orig data) → CREATE + ADD
 *   Case 2: Any change → smart delta for qty-only, REMOVE old + ADD new for anything else
 *
 * @param {Array} row - 19-element array (0-indexed)
...
function computeRowActions_(row, rowIndex) {
  ...
  // row[2] = Cost (read-only), row[3] = Type (Katana category, read-only)
  var site = String(row[4]).trim();
  var location = String(row[5]).trim();
  var lotTracked = String(row[6]).trim();
  var lot = String(row[7]).trim();
  var dateCode = '';
  var uom = String(row[9]).trim();
  // row[10] = K.Qty (read-only Katana qty, not used for push logic)
  var origQtyRaw = row[11];   // Orig WASP Qty (hidden baseline)
  var qtyRaw = row[12];       // W.Qty (editable)
  var origSite = String(row[13]).trim();
  var origLocation = String(row[14]).trim();
  // row[15] = Match, row[16] = Status
  var origLot = String(row[17]).trim();
  var origDateCode = '';

  // Handle Date objects in dateCode fields (Sheets auto-converts date-like text)
  if (row[8] instanceof Date) {
    var dcY = row[8].getFullYear();
    var dcM = row[8].getMonth() + 1;
    var dcD = row[8].getDate();
    dateCode = dcY + '-' + (dcM < 10 ? '0' + dcM : dcM) + '-' + (dcD < 10 ? '0' + dcD : dcD);
  } else {
    dateCode = String(row[8] || '').trim();
    if (dateCode.indexOf('T') > -1) dateCode = dateCode.split('T')[0];
  }

  if (row[18] instanceof Date) {
    var odcY = row[18].getFullYear();
    var odcM = row[18].getMonth() + 1;
    var odcD = row[18].getDate();
    origDateCode = odcY + '-' + (odcM < 10 ? '0' + odcM : odcM) + '-' + (odcD < 10 ? '0' + odcD : odcD);
  } else {
    origDateCode = String(row[18] || '').trim();
...
function executeAction_(action) {
  ...
  if ((action.type === 'ADD' || action.type === 'REMOVE') && action.lot && !action._dateNormalized) {
    action = resolveExistingWaspLotDateCode_(action);
  }
...
function detectDeletedRows_(sheet, lastRow, numCols) {
  ...
  if (lastRow >= 2) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var site = String(data[i][4]).trim();
      var loc = String(data[i][5]).trim();
      var lot = String(data[i][7]).trim();
...
function pushBatchPendingRows_(ss) {
  ...
  for (var j = 0; j < pendingRows.length; j++) {
    ...
    if (allSuccess) {
      ...
      data[rowIndex][19] = data[rowIndex][11]; // Orig W.DateCode = W.DateCode
      ...
      var bLogOrigQty = sheetNumberValue_(row[14]);
      var bLogNewQty = sheetNumberValue_(row[8]);
  ...
  for (var dri = 0; dri < deleteRows.length; dri++) {
    ...
    var delOrigQty = sheetNumberValue_(delRow[14]);
    var delOrigSite = String(delRow[15] || '').trim();
    var delOrigLocation = String(delRow[16] || '').trim();
    var delOrigLot = String(delRow[18] || '').trim();
    var delOrigDateCode = sheetDateValue_(delRow[19]);
    var delHasOrig = !!(delOrigSite || delOrigLocation || delOrigLot || delOrigDateCode || Math.abs(delOrigQty) > 0.01);

    var delSite = delHasOrig ? delOrigSite : String(delRow[6]).trim();
    var delLocation = delHasOrig ? delOrigLocation : String(delRow[7]).trim();
    var delQty = delHasOrig ? delOrigQty : sheetNumberValue_(delRow[8]);
    var delLot = delHasOrig ? delOrigLot : String(delRow[10]).trim();
    var delDateCode = delHasOrig ? delOrigDateCode : sheetDateValue_(delRow[11]);
...
function pushKatanaSyncRows_(ss) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var itemType = String(data[i][14] || data[i][3] || '').trim();   // hidden K.Type preferred, display category fallback
    var katanaRoute = getKatanaPushTarget_(katanaLoc, itemType);
  ...
  for (var si = 0; si < skuList.length; si++) {
    ...
    if (!createResult.success) {
      ...
      // Category not found in WASP — retry without category
      if (createErr.indexOf('-57006') > -1 || createErr.indexOf('does not exist') > -1 && createErr.indexOf('Category') > -1) {
        Logger.log('Katana CREATE for ' + sku + ': category "' + (katCategory || '') + '" not in WASP — retrying without category');
        delete createItem.CategoryDescription;
        createResult = waspApiCall(getWaspBase() + 'ic/item/createInventoryItems', [createItem]);
        itemCreated = createResult.success;
        if (!createResult.success) {
          createErr = String(createResult.response || '').substring(0, 200);
        } else {
          createErr = '';
    ...
    if (!createResult.success && !itemCreated) {
      createErr = createErr || String(createResult.response || '').substring(0, 200);
...
function getKatanaPushTarget_(katanaLoc, itemType) {
  if (typeof getKatanaSyncTarget_ === 'function') {
...
function resolveExistingWaspLotDateCode_(action) {
  if (!action || !action.lot) return action;
  ...
  var normalized = {
    ...
    _rowLot: action._rowLot,
    _rowDateCode: action._rowDateCode,
    _retried: action._retried,
    _lotRetried: action._lotRetried,
    _dateNormalized: true
  };

  try {
    var lookup = waspFetch('ic/item/inventorysearch', {
      ItemNumber: action.sku,
      SiteName: action.site || WASP_DEFAULT_SITE,
      LocationCode: action.location || ''
    });
    var rows = lookup.Data || lookup.data || [];
    var best = null;
    for (var i = 0; i < rows.length; i++) {
      var item = rows[i];
      var itemLot = String(item.Lot || item.LotNumber || item.BatchNumber || item.lot || '').trim();
      if (itemLot !== String(action.lot || '').trim()) continue;

      var itemLocation = String(item.LocationCode || item.Location || item.locationCode || '').trim();
      var itemDateCode = safeDate(item.DateCode || item.ExpiryDate || item.ExpDate || item.dateCode || '');
      if (!best) {
        best = { location: itemLocation, dateCode: itemDateCode };
        continue;
      }
      if (itemLocation === String(action.location || '').trim() && best.location !== itemLocation) {
        best = { location: itemLocation, dateCode: itemDateCode };
        continue;
      }
      if (datesWithinTolerance_(action.dateCode || action._rowDateCode || '', itemDateCode, 1) &&
          !datesWithinTolerance_(action.dateCode || action._rowDateCode || '', best.dateCode || '', 1)) {
        best = { location: itemLocation, dateCode: itemDateCode };
      }
    }

    if (!best || !best.dateCode) return normalized;
    if (!normalized.dateCode || datesWithinTolerance_(normalized.dateCode, best.dateCode, 1)) {
      normalized.dateCode = best.dateCode;
      if (!normalized._rowDateCode || datesWithinTolerance_(normalized._rowDateCode, best.dateCode, 1)) {
        normalized._rowDateCode = best.dateCode;
      }
    }
  } catch (e) {
    Logger.log('Lot date normalization failed for ' + action.sku + '/' + action.lot + ': ' + e.message);
  }
...
function autoAdjustWasp() {
  ...
  for (var i = 0; i < data.length; i++) {
    var row = data[i];
    var sku = String(row[0] || '').trim();
    var match = String(row[15] || '').trim();
    if (!sku || sku.indexOf(' > ') > -1 || match !== 'ADJUSTABLE') continue;

    var site = String(row[4] || '').trim();
    var location = String(row[5] || '').trim();
    var kQty = (row[10] instanceof Date) ? 0 : (parseFloat(row[10]) || 0);
    var wQty = (row[12] instanceof Date) ? 0 : (parseFloat(row[12]) || 0);
