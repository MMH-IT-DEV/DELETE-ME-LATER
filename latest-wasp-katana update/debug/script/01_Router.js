...
function doPost(e) {
  ...
  // Transient errors (API fetch failures, lock timeouts) are downgraded to success
  // to avoid flooding Slack with non-actionable alerts. Only permanent errors alert.
  try {
    var hbMsg = (result && result.message) || '';
    var isTransient = hbMsg.indexOf('Failed to fetch') >= 0
      || hbMsg.indexOf('Lock timeout') >= 0
      || hbMsg.indexOf('already in progress') >= 0
      || hbMsg.indexOf('Failed to fetch MO') >= 0;
    var hbStatus  = (result && result.status === 'error' && !isTransient) ? 'error' : 'success';
...
function routeWebhook(payload) {
  ...
  if (action === 'purchase_order_row.received') {
    return handlePurchaseOrderRowReceived(payload);
  }
  ...
  if (action === 'health_check_report') {
    return handleHealthCheckReport(payload);
  }
