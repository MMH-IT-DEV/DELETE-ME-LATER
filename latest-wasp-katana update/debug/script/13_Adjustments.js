 ...
 * Google-sheet adjustments are operational only: they update WASP but do not
 * create Activity/F2 audit rows. Manual WASP-originated adjustments remain
 * logged through the F2 callout flow in 03_WaspCallouts.gs.
 *
...
function processAdjustments() {
  ...
  for (var p = 0; p < pendingRows.length; p++) {
    ...
    markSyncedToWasp(sku, location, effectiveAction === 'ADD' ? 'add' : 'remove', site);
...
function retryMarkedItems() {
  ...
  for (var c = 0; c < checkedRows.length; c++) {
    ...
    markSyncedToWasp(parsed.sku, location, action === 'ADD' ? 'add' : 'remove', site);
    ...
    var subAction = buildActivityCompactMeta_(site, location, lot, expiry);
  ...
  if (f2SubItems.length > 0) {
    ...
    var retrySiteSummary = buildActivitySiteSummary_(retrySitesSeen);
    logActivity('F2', f2Detail, f2Status, buildActivitySourceActionContext_('Activity', 'retry', retrySiteSummary), f2SubItems);
  }
...
function onOpenRetryMenu() {
  ...
  ui.createMenu('Inventory Sync')
    .addItem('F5: Build Recovery Report', 'buildF5RecoveryReportToday')
    .addItem('F5: Finalize Recovery', 'finalizeF5RecoveryReport')
    .addSeparator()
    .addItem('Test Katana API', 'testKatanaApiConnection_')
    .addItem('Test WASP API', 'testWaspApiConnection_')
    .addItem('Test ShipStation API', 'testShipStationApiConnection_')
    .addToUi();
}

/**
 * Test Katana API connection — shows result in toast.
 */
function testKatanaApiConnection_() {
  var ss = SpreadsheetApp.openById(CONFIG.DEBUG_SHEET_ID);
  try {
    var result = katanaApiCall('locations');
    var locations = result && result.data ? result.data : (result || []);
    var names = [];
    for (var i = 0; i < locations.length; i++) { names.push(locations[i].name); }
    ss.toast('Katana API: OK\nLocations: ' + names.join(', '), 'Connection Test', 10);
  } catch (e) {
    ss.toast('Katana API: FAILED\n' + e.message, 'Connection Test', 10);
  }
}

/**
 * Test WASP API connection — shows result in toast.
 */
function testWaspApiConnection_() {
  var ss = SpreadsheetApp.openById(CONFIG.DEBUG_SHEET_ID);
  try {
    var result = waspApiCall(
      CONFIG.WASP_BASE_URL + '/public-api/ic/item/infosearch',
      { ItemNumber: '', AltItemNumber: '', ItemDescription: '' }
    );
    if (result.success) {
      ss.toast('WASP API: OK\nEndpoint responded successfully', 'Connection Test', 10);
    } else {
      ss.toast('WASP API: ERROR\n' + (result.response || 'Unknown error').substring(0, 200), 'Connection Test', 10);
    }
  } catch (e) {
    ss.toast('WASP API: FAILED\n' + e.message, 'Connection Test', 10);
  }
}

/**
 * Test ShipStation API connection — shows result in toast.
 */
function testShipStationApiConnection_() {
  var ss = SpreadsheetApp.openById(CONFIG.DEBUG_SHEET_ID);
  try {
    var result = callShipStationAPI('/orders?pageSize=1', 'GET');
    if (result.code === 200) {
      ss.toast('ShipStation API: OK\nHTTP ' + result.code, 'Connection Test', 10);
    } else {
      ss.toast('ShipStation API: ERROR\nHTTP ' + result.code, 'Connection Test', 10);
    }
  } catch (e) {
    ss.toast('ShipStation API: FAILED\n' + e.message, 'Connection Test', 10);
  }
}

/**
 * Remove debug/testing tabs from the spreadsheet.
 * Run once from the menu, then this menu item can be removed.
 */
function removeDebugTabs_() {
  var ss = SpreadsheetApp.openById(CONFIG.DEBUG_SHEET_ID);
  var tabsToRemove = [
    'Activity QA', 'Activity QA Report', 'Activity QA Katana',
    'Activity QA Katana Report', 'Activity QA Inputs', 'MO Trace'
  ];
  var removed = [];
  for (var i = 0; i < tabsToRemove.length; i++) {
    var sheet = ss.getSheetByName(tabsToRemove[i]);
    if (sheet) {
      ss.deleteSheet(sheet);
      removed.push(tabsToRemove[i]);
    }
  }
  var msg = removed.length > 0
    ? 'Removed ' + removed.length + ' tabs: ' + removed.join(', ')
    : 'No debug tabs found to remove.';
  ss.toast(msg, 'Cleanup', 10);
}
