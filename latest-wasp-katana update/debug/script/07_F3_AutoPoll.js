...
function mapKatanaToWaspSite(katanaLocation) {
  ...
  if (useConfigDrivenMap) {
    var configuredMap = {};
    var configSources = [KATANA_LOCATION_TO_WASP, typeof F3_LOCATION_OVERRIDES !== 'undefined' ? F3_LOCATION_OVERRIDES : null];
...
function getF3LocationRank_(siteName, locationCode, preferredLocation) {
  ...
  var priorities = {
    ...
    'MMH Mayfair': ['PRODUCTION', 'QA-Hold-1', 'QA-Hold-2', 'QA-Hold-4', 'QA-Hold-5'],
...
function pollKatanaStockTransfers() {
  ...
  try {
    if (!lock.tryLock(F3_CONFIG.POLL_LOCK_WAIT_MS)) {
      return { status: 'skipped', message: 'F3 poll already running' };
    }
    ...
    var nextCursor = computeF3NextCursor_(lastSync, transfers);

    if (!transfers || transfers.length === 0) {
      cache.put(F3_CONFIG.CACHE_KEY_LAST_SYNC, nextCursor, 21600);
      updateF3Summary();
...
function fetchKatanaStockTransfers(since) {
  ...
  for (var page = 1; page <= maxPages; page++) {
    var params = [];
    if (since) params.push('updated_at_min=' + encodeURIComponent(since));
    params.push('include=stock_transfer_rows');
    params.push('limit=100');
    params.push('page=' + page);
    params.push('sort=-updated_at');

    var pageEndpoint = endpoint + '?' + params.join('&');
    var result = katanaApiCall(pageEndpoint);
    var rows = result && result.data ? result.data : [];
    if (!rows || rows.length === 0) break;

    for (var i = 0; i < rows.length; i++) {
      var row = rows[i];
      var rowId = String(row && row.id ? row.id : '');
...
function computeF3NextCursor_(lastSync, transfers) {
  var overlapMs = Math.max(0, Number(F3_CONFIG.CURSOR_OVERLAP_SECONDS || 0)) * 1000;
  var latestMs = 0;

  if (transfers && transfers.length) {
    for (var i = 0; i < transfers.length; i++) {
      var transfer = transfers[i] || {};
      var updatedAt = transfer.updated_at || transfer.created_at || '';
      var updatedMs = updatedAt ? new Date(updatedAt).getTime() : 0;
      if (updatedMs > latestMs) latestMs = updatedMs;
    }
  }

  if (!latestMs) {
    latestMs = lastSync ? new Date(lastSync).getTime() : Date.now();
  }
  if (!latestMs || isNaN(latestMs)) latestMs = Date.now();

  var cursorMs = Math.max(0, latestMs - overlapMs);
...
function processStockTransfer(transfer) {
  ...
  Logger.log('F3 processing: ' + stNumber + ' status=' + status + ' (raw=' + (transfer.status || '') + ')');
  ...
  if (shouldReverse) {
    if (existingRow) {
      ...
      // Already reversed or failed — don't re-process
      if (rvWaspSt === 'Reversed' || rvWaspSt === 'Failed') {
        return { processed: true, synced: false };
      }
      // Not previously synced — DON'T set Skip. Leave status as-is so the ST
      // can be processed later if it transitions to 'received'.
      // This prevents draft (newly created) STs from being permanently blocked.
    }
    return { processed: true, synced: false };
  }
...
function syncSTToWasp(stId, stNumber, fromLocKatana, toLocKatana, rows, sheetRow, katanaStatus, isRetry) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (bt.length > 1) {
      ...
      markSyncedToWasp(sku, fromWasp.location, 'remove', fromWasp.site);
      markSyncedToWasp(sku, toWasp.location, 'add', toWasp.site);
  ...
  if (shouldLogActivity) {
    ...
    var f3ShowLineRoute = f3HeaderLocs.fromLocation === 'Mixed source' || f3HeaderLocs.toLocation === 'Mixed dest';
    var f3pSubItems = [];
    ...
    f3pContext = buildActivityTransferContext_('Katana poll', 'transfer', f3HeaderLocs.fromSite, f3HeaderLocs.toSite);
...
function reverseSTFromWasp(stId, stNumber, fromLocKatana, toLocKatana, rows, sheetRow) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (bt.length > 1) {
      ...
      markSyncedToWasp(sku, toWasp.location, 'remove', toWasp.site);
      markSyncedToWasp(sku, fromWasp.location, 'add', fromWasp.site);
  ...
  if (syncedNow > 0 || failedCount > 0) {
    var f3RevHeaderLocs = buildF3HeaderLocations_(successItems, errors, toWasp, fromWasp);
    var f3RevShowLineRoute = f3RevHeaderLocs.fromLocation === 'Mixed source' || f3RevHeaderLocs.toLocation === 'Mixed dest';
    var f3SubItems = [];
    for (var s = 0; s < successItems.length; s++) {
      var sItem = successItems[s];
      if (sItem.multiBatch && sItem.multiBatchFirst) {
    ...
    f3pContext = buildActivityTransferContext_('Katana poll', 'transfer revert', toWasp.site, fromWasp.site);
...
function syncSTLineItem(stId, stNumber, sku, qty, lot, expiry, fromWasp, toWasp) {
  ...
  markSyncedToWasp(sku, actualFromWasp.location, 'remove', actualFromWasp.site);
  markSyncedToWasp(sku, toWasp.location, 'add', toWasp.site);
...
function clearF3SheetAndPoll() {
  // Clear all data rows from StockTransfers tab (keep header)
  var ss = SpreadsheetApp.openById(F3_CONFIG.SHEET_ID);
  var sheet = ss.getSheetByName(F3_CONFIG.TAB_NAME);
  if (sheet && sheet.getLastRow() > 1) {
    sheet.deleteRows(2, sheet.getLastRow() - 1);
    Logger.log('Cleared ' + F3_CONFIG.TAB_NAME + ' tab');
  }
  // Reset cursor and poll fresh
  CacheService.getScriptCache().remove(F3_CONFIG.CACHE_KEY_LAST_SYNC);
  Logger.log('F3 cursor reset to MIN_CREATED_DATE: ' + F3_CONFIG.MIN_CREATED_DATE);
  var result = pollKatanaStockTransfers();
  Logger.log(JSON.stringify(result, null, 2));
}

function debugSingleST(stNumber) {
  if (!stNumber) { Logger.log('Usage: set stNumber in code'); return; }
  var transfers = fetchKatanaStockTransfers(F3_CONFIG.MIN_CREATED_DATE);
  Logger.log('Fetched ' + transfers.length + ' transfers');
  var found = null;
  for (var i = 0; i < transfers.length; i++) {
    var t = transfers[i];
    var tNum = t.stock_transfer_number || ('ST-' + t.id);
    if (tNum === stNumber) { found = t; break; }
  }
  if (!found) { Logger.log('ST ' + stNumber + ' not found in API response'); return; }
  Logger.log('Found: ' + stNumber + ' status=' + (found.status || ''));
  var rows = found.stock_transfer_rows || [];
  ...
  for (var r = 0; r < rows.length; r++) {
    ...
    var bt = row.batch_transactions || [];
    Logger.log('  Row ' + r + ': sku=' + sku + ' qty=' + (row.quantity || 0) + ' batch_transactions=' + bt.length);
    for (var b = 0; b < bt.length; b++) {
      Logger.log('    BT ' + b + ': batch_id=' + (bt[b].batch_id || '') + ' qty=' + (bt[b].quantity || ''));
    }
  }
  // Check StockTransfers sheet
  var existingRow = findSTInSheet(found.id);
  Logger.log('StockTransfers sheet row: ' + (existingRow || 'NOT FOUND'));
  if (existingRow) {
    Logger.log('WASP Status: ' + getSTWaspStatus(existingRow));
  }
  // Try processing
  Logger.log('--- Processing ---');
  var result = processStockTransfer(found);
  Logger.log('Result: ' + JSON.stringify(result));
}

function debugST266() { debugSingleST('ST-266'); }

function resetSTStatus(stNumber) {
  var ss = SpreadsheetApp.openById(F3_CONFIG.SHEET_ID);
  var sheet = ss.getSheetByName(F3_CONFIG.TAB_NAME);
  if (!sheet || sheet.getLastRow() < 2) { Logger.log('No data'); return; }
  var data = sheet.getRange(2, 1, sheet.getLastRow() - 1, 11).getValues();
  for (var i = 0; i < data.length; i++) {
    if (String(data[i][1]).trim() === stNumber) {
      var row = i + 2;
      sheet.getRange(row, F3_CONFIG.COLS.WASP_STATUS).setValue('');
      sheet.getRange(row, F3_CONFIG.COLS.NOTES).setValue('Reset for reprocessing');
      Logger.log('Reset ' + stNumber + ' at row ' + row + ' (was: ' + data[i][8] + ')');
      return;
    }
  }
  Logger.log(stNumber + ' not found in StockTransfers tab');
}

function resetST266() { resetSTStatus('ST-266'); }

function resetF3CursorAndPoll() {
  CacheService.getScriptCache().remove(F3_CONFIG.CACHE_KEY_LAST_SYNC);
  Logger.log('F3 cursor reset to MIN_CREATED_DATE: ' + F3_CONFIG.MIN_CREATED_DATE);
  var result = pollKatanaStockTransfers();
  Logger.log(JSON.stringify(result, null, 2));
