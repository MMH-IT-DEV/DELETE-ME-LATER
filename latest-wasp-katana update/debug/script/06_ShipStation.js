 ...
 *
 * Previously this fetched the resource URL directly and called processShipment()
 * for each shipment in a loop. When a user prints multiple labels at once,
 * ShipStation fires one SHIP_NOTIFY per label simultaneously, creating concurrent
 * GAS executions that raced to write to the Activity sheet at the same row.
 *
 * Fix: delegate immediately to processWebhookQueue(), which holds a CacheService
 * guard (ss_poll_running) so only one execution processes at a time. Concurrent
 * webhook calls hit the guard and return immediately. The 1-minute time trigger
 * on processWebhookQueue() remains active as a true safety net.
 *
 * @param {string} resourceUrl - Full ShipStation resource URL (logged only)
 * @returns {Object} Processing summary from processWebhookQueue()
 */
...
function processShipment(shipment, execId) {
  ...
  // Set dedup cache IMMEDIATELY to prevent concurrent runs from double-processing.
  // Previously this was only set before processing when !useLedgerRetryGuard,
  // leaving a race window where two concurrent processWebhookQueue calls could
  // both pass the dedup check for the same shipmentId.
  cache.put(cacheKey, 'processing', 86400);
  ...
  for (var i = 0; i < shipmentItems.length; i++) {
    ...
    for (var p = 0; p < itemsToProcess.length; p++) {
      ...
      markSyncedToWasp(proc.sku, 'SHOPIFY', 'remove', 'MMH Kelowna');
...
function processVoid(shipment, execId) {
  ...
  // Set dedup cache IMMEDIATELY to prevent concurrent void runs from double-processing.
  // Same fix as processShipment — close the race window before WASP calls.
  cache.put(cacheKey, 'processing', 86400);
  ...
  for (var i = 0; i < shipmentItems.length; i++) {
    ...
    for (var r = 0; r < returnItems.length; r++) {
      ...
      // No ledger row found means the shipment was never processed (e.g. voided before
      // processWebhookQueue ran — typically within ~60 seconds of label printing).
      // Skip the inventory reversal: there is nothing to put back.
      if (useLedgerRetryGuard && ledgerLine && !previousLedgerStatus) {
        voidedItems.push({
          sku: ret.sku,
          qty: ret.qty,
          success: true,
          preserveLedgerStatus: false,
          error: 'No ledger entry — shipment was never deducted',
          action: 'SHOPIFY',
          status: 'Skipped',
          qtyColor: 'grey'
        });
      ...
      markSyncedToWasp(ret.sku, 'SHOPIFY', 'add', 'MMH Kelowna');
...
function processWebhookQueue() {
  ...
  pollCache.put('ss_poll_running', '1', 300); // 5 min safety TTL — prevents concurrent runs
