function refreshShopifyActivity() {
  ...
  for (var j = 0; j < accountRows.length; j++) {
    ...
    if (normalizePresenceState_(account.CURRENT_STATE) !== 'Revoked') {
      account.CURRENT_STATE = 'Access Granted';
    }
...
function generateReviewChecks() {
  ...
  // Pre-read all review rows once — avoids re-reading the sheet per account
  var allReviewRows = getReviewRows_();
  var reviewsByIdentity = {};
  for (var r = 0; r < allReviewRows.length; r++) {
    var key = reviewReusableIdentityKey_(allReviewRows[r].data);
    if (!key) { continue; }
    if (!reviewsByIdentity[key]) { reviewsByIdentity[key] = []; }
    reviewsByIdentity[key].push(allReviewRows[r]);
  }
  ...
  for (var i = 0; i < accountRows.length; i++) {
    ...
    var trigger = determineReviewTrigger_(account, today);
    var isScheduledTrigger = isFutureScheduledReviewTrigger_(trigger, todayStart);
    ...
    var relatedRows = reviewsByIdentity[accountIdentity] || [];
    var reviewRow = chooseCurrentReviewRowForAccount_(relatedRows) || chooseLatestDoneReviewRowForAccount_(relatedRows);
    ...
    if (reviewRow && previousStatus === 'Waiting') {
      review.DECISION_STATUS = 'Open';
      review.NEXT_REVIEW_DUE = isScheduledTrigger ? trigger.reviewDue : '';
    ...
    account.NEXT_REVIEW_DUE = trigger.reviewDue || account.NEXT_REVIEW_DUE || today;
...
function isFutureScheduledReviewTrigger_(trigger, todayStart) {
  if (!trigger || !(trigger.reviewDue instanceof Date) || isNaN(trigger.reviewDue)) {
    return false;
  }
  // Match the 2-day open window used in isScheduledReviewRow_
  var openWindow = new Date(todayStart.getTime() + (2 * 86400000));
  var reviewDueDay = new Date(trigger.reviewDue.getFullYear(), trigger.reviewDue.getMonth(), trigger.reviewDue.getDate());
  return reviewDueDay.getTime() > openWindow.getTime();
}
