var GMP_SLASH_CONFIG = {
  ACCESS_COMMAND: '/system-access',
  ACCESS_ALIAS_COMMAND: '/system-request-access',
  ISSUE_COMMAND: '/system-issue',
...
function doGet() {
  return ContentService.createTextOutput('System security tracker endpoint active.');
}

function routeSecurityTrackerPost_(e) {
  try {
    // Slash commands (modal opens) skip verification for speed — the modal is
    // just a form display, no data is written. Submissions still verify below.
    if (e.parameter &&
    ...
    // Submissions and interactions still verify
    ...
    return slackAckText_(':warning: System security tracker failed: ' + err.message);
  }
}
...
function handleAccessSlashCommand_(params) {
  ...
  if (!result.ok) {
    return slackAckText_(':warning: Could not open /system-access modal: ' + (result.detail || result.error || 'unknown error'));
  }
  return slackAck_();
}
...
function handleIssueSlashCommand_(params) {
  ...
  if (!isUserAllowedForIssueCommand_(params.user_id)) {
    return slackAckText_(':no_entry: You don\'t have access to this command. Contact IT if you think this is a mistake.');
  }

  var result = openSlackModal_(params.trigger_id, buildIssueModal_(params));
  if (!result.ok) {
    return slackAckText_(':warning: Could not open /system-issue modal: ' + (result.detail || result.error || 'unknown error'));
  }
  return slackAck_();
}

function isUserAllowedForIssueCommand_(userId) {
  var groupId = normalizeText_(
    PropertiesService.getScriptProperties().getProperty(GMP_CONFIG.ISSUE_COMMAND_GROUP_PROPERTY)
  );

  // No group configured — open to everyone
  if (!groupId) {
    return true;
  }

  if (!normalizeText_(userId)) {
    return false;
  }

  // Cache the member list for 5 minutes so warm calls skip the Slack API round-trip
  var cache = CacheService.getScriptCache();
  var cacheKey = 'ISSUE_CMD_MEMBERS_' + groupId;
  var members;
  ...
  if (!members) {
    try {
      var response = UrlFetchApp.fetch('https://slack.com/api/usergroups.users.list?usergroup=' + groupId, {
        method: 'get',
      ...
      if (!result.ok) {
        Logger.log('isUserAllowedForIssueCommand_ Slack error: ' + result.error);
        return true; // fail-open
      }
      members = result.users || [];
      try { cache.put(cacheKey, JSON.stringify(members), 300); } catch (err) {}
    } catch (err) {
      Logger.log('isUserAllowedForIssueCommand_ error: ' + err.message);
      return true; // fail-open
    }
  }
...
function handleAccessSubmission_(interaction) {
  var records = buildAccessRequestRecords_(interaction);
  if (!records.length) {
    return slackAckText_(':warning: Please select at least one platform before submitting.');
  }

  var props = PropertiesService.getScriptProperties();

  // Read dedup cache once (not once per platform)
  var cache = {};
  try {
  ...
  // Build all queue entries in memory, update cache in memory
  var toWrite = {};
  var now = Date.now();
  for (var i = 0; i < records.length; i++) {
    var record = records[i];
    var submissionKey = normalizeText_(record._SUBMISSION_KEY);
    if (submissionKey) {
      var cacheKey = hashSecurityValue_(submissionKey);
      if (cache[cacheKey]) {
        Logger.log('Skipping duplicate access submission: ' + submissionKey);
        continue;
      }
      cache[cacheKey] = { recordId: normalizeText_(record.REQUEST_ID), at: now };
    }
    toWrite['PENDING_SECURITY_JOB_ACCESS_' + now + '_' + i + '_' + Utilities.getUuid().slice(0, 8)] =
      JSON.stringify({ kind: 'access', record: sanitizeRecordForQueue_(record), createdAt: new Date().toISOString() });
  }

  // Single batch write regardless of how many platforms were selected
  if (Object.keys(toWrite).length) {
    toWrite['SECURITY_SUBMISSION_CACHE'] = JSON.stringify(cache);
...
function handleIssueSubmission_(interaction) {
  ...
  var cache = {};
  try {
    var raw = props.getProperty('SECURITY_SUBMISSION_CACHE');
    if (raw) { cache = JSON.parse(raw) || {}; }
  } catch (err) {}
  pruneSecuritySubmissionCache_(cache);

  var submissionKey = normalizeText_(incidentRecord._SUBMISSION_KEY);
  if (submissionKey) {
    var cacheKey = hashSecurityValue_(submissionKey);
    if (cache[cacheKey]) {
      Logger.log('Skipping duplicate incident submission: ' + submissionKey);
      return slackClearModal_();
    }
    cache[cacheKey] = { recordId: incidentRecord.INCIDENT_ID, at: Date.now() };
  }

  var toWrite = {};
  toWrite['PENDING_SECURITY_JOB_INCIDENT_' + Date.now() + '_' + Utilities.getUuid().slice(0, 8)] =
    JSON.stringify({ kind: 'incident', record: sanitizeRecordForQueue_(incidentRecord), createdAt: new Date().toISOString() });
  toWrite['SECURITY_SUBMISSION_CACHE'] = JSON.stringify(cache);
  props.setProperties(toWrite);
...
function resolveSubmitterFullName_(user) {
  if (!user) { return ''; }
  var userId = normalizeText_(user.id);
  if (userId) {
    var cache = CacheService.getScriptCache();
    var cacheKey = 'SLACK_FULL_NAME_' + userId;
    var cached = cache.get(cacheKey);
    if (cached) { return cached; }
    try {
      var resolved = lookupSlackUserDisplayName_(userId);
      if (resolved && resolved !== userId) {
        try { cache.put(cacheKey, resolved, 1800); } catch (e) {}
        return resolved;
      }
    } catch (err) {
      Logger.log('resolveSubmitterFullName_ failed: ' + err.message);
    }
  }
  return normalizeText_(user.real_name || user.name || user.username || userId);
}

function buildAccessRequestRecords_(interaction) {
  ...
  var submitterName = normalizeText_((interaction.user && (interaction.user.name || interaction.user.username)) || '');
  var submitterId   = normalizeText_(interaction.user && interaction.user.id);

  var records = [];
  var platforms = GMP_SCHEMA.platforms;

  for (var i = 0; i < platforms.length; i++) {
    var platform = platforms[i];
    var accessLevel = getSelectedValue_(values, platformBlockId_(platform), platformActionId_(platform));
    if (!accessLevel) {
      continue;
    }
    ...
    record.QUEUE_PRIORITY = deriveRequestPriority_(requestType, 'Yes');
    record.REQUEST_TYPE = requestType;
    record.SLACK_USER = submitterName;
    record.TARGET_USER = targetUser;
    record.COMPANY_EMAIL = companyEmail;
    record.DEPT = department;
    record.GMP_SYSTEM = platform;
    record.ACCESS_LEVEL = accessLevel;
    record.GMP_IMPACT = 'Yes';
    record.REASON = reason;
    record.MANAGER = submitterName;
    record.IT_OWNER = '';
    ...
    record._SUBMITTER_ID = submitterId;
    record._SUBMISSION_KEY = buildSlackInteractionKey_(interaction, 'access') + '|' + platform;
...
function platformBlockId_(platform) {
  return 'blk_platform_' + platform.toLowerCase().replace(/[^a-z0-9]/g, '_');
}

function platformActionId_(platform) {
  return 'platform_' + platform.toLowerCase().replace(/[^a-z0-9]/g, '_');
}
...
function buildIncidentRecord_(interaction) {
  ...
  record.SLACK_USER = normalizeText_((interaction.user && (interaction.user.name || interaction.user.username)) || '');
  record._SUBMITTER_ID = normalizeText_(interaction.user && interaction.user.id);
...
function buildAccessRequestBlocks_(record, rowNumber) {
  return [
    buildSectionBlock_('*New system access request*'),
    buildSectionBlock_(
    ...
    buildSectionBlock_(
      '*System:* `' + record.GMP_SYSTEM + '`\n' +
        ...
        '*System Impact:* `' + record.GMP_IMPACT + '`'
    ),
...
function buildIncidentBlocks_(record, rowNumber) {
  var title = record._SOURCE_COMMAND === GMP_SLASH_CONFIG.ISSUE_ALIAS_COMMAND ? 'New IT alert' : 'New system issue report';
  return [
...
function buildAccessRequestModal_(params) {
  ...
  var accessLevelOptions = staticOptions_(['Admin', 'Full Access', 'Read-Write', 'Read-Only', 'Limited', 'Remove']);
  ...
  for (var i = 0; i < platforms.length; i++) {
    ...
    platformBlocks.push({
      type: 'input',
      block_id: platformBlockId_(platform),
      optional: true,
      label: { type: 'plain_text', text: platform },
      element: {
        type: 'static_select',
        action_id: platformActionId_(platform),
        placeholder: { type: 'plain_text', text: '— Not needed —' },
  ...
  var topBlocks = [
    ...
    {
      type: 'context',
      elements: [{ type: 'mrkdwn', text: '*Platform Access* — Select an access level for each platform needed. Leave blank to skip.' }]
    }
  ];
  ...
  return {
    ...
    title: { type: 'plain_text', text: 'System Access' },
    submit: { type: 'plain_text', text: 'Submit' },
    close: { type: 'plain_text', text: 'Cancel' },
    blocks: topBlocks.concat(platformBlocks)
  };
}
...
function buildIssueModal_(params) {
  ...
  var title = isItAlert ? 'IT Alert' : 'System Issue';
  var introText = isItAlert ?
    ...
    ':rotating_light: Report a system issue once with enough detail for follow-up. This posts to the support thread and logs a single incident row.';
  return {
    type: 'modal',
    ...
    blocks: [
      ...
      { type: 'divider' },
      {
        ...
        element: {
          ...
          placeholder: { type: 'plain_text', text: 'Katana Inventory, Shopify Admin, Google Drive System Folder...' }
        }
      },
      {
        type: 'input',
...
function tryPostInitialAccessNotification_(requestRecord, rowNumber) {
  try {
    var slackMessage = postSlackMessageWithFallback_(
      ...
      'New system access request ' + requestRecord.REQUEST_ID,
...
function tryPostInitialIncidentNotification_(incidentRecord, rowNumber) {
  try {
    var slackMessage = postSlackMessageWithFallback_(
      ...
      'New system issue ' + incidentRecord.INCIDENT_ID,
...
function processPendingSecurityIntake_() {
  ...
  try {
    ...
    // Split access jobs from others so access rows are batch-inserted
    // with a single sort at the end instead of one sort per row.
    var accessJobKeys = [];
    var otherJobKeys = [];
    for (var i = 0; i < keys.length; i++) {
      try {
        var job = JSON.parse(allProps[keys[i]]);
        if (job && job.kind === 'access') {
          accessJobKeys.push(keys[i]);
        } else {
          otherJobKeys.push(keys[i]);
        }
      } catch (err) {
        otherJobKeys.push(keys[i]);
      }
    ...
    // Insert all access rows without intermediate sorts or workbook re-checks
    for (var j = 0; j < accessJobKeys.length; j++) {
      try {
        processQueuedSecurityJob_(accessJobKeys[j], allProps[accessJobKeys[j]], true);
      } catch (err) {
        Logger.log('Queued access job failed: ' + accessJobKeys[j] + ' | ' + err.message);
      }
    }
    // One sort covers all inserted access rows
    if (accessJobKeys.length) {
      sortRequestSection_();
    }

    // Incident and other jobs processed normally (unchanged behaviour)
    for (var k = 0; k < otherJobKeys.length; k++) {
      try {
        processQueuedSecurityJob_(otherJobKeys[k], allProps[otherJobKeys[k]]);
      } catch (err) {
        Logger.log('Queued security job failed: ' + otherJobKeys[k] + ' | ' + err.message);
      }
    }
  } finally {
    lock.releaseLock();
...
function processQueuedSecurityJob_(propertyKey, rawValue, skipInitAndSort) {
  var props = PropertiesService.getScriptProperties();
  ...
  if (job.kind === 'access') {
    processQueuedAccessJob_(job.record, skipInitAndSort);
    props.deleteProperty(propertyKey);
    return;
  }
...
function processQueuedAccessJob_(rawRecord, skipInitAndSort) {
  var requestRecord = hydrateQueuedRecord_(rawRecord);

  // Resolve full name in background — no 3-second limit here
  var submitterId = normalizeText_(requestRecord._SUBMITTER_ID);
  if (submitterId) {
    try {
      var fullName = resolveSubmitterFullName_({ id: submitterId, name: requestRecord.SLACK_USER });
      if (fullName) {
        requestRecord.SLACK_USER = fullName;
        requestRecord.MANAGER    = fullName;
      }
    } catch (err) {
      Logger.log('Name resolution skipped: ' + err.message);
    }
  }

  var existingRow = findRequestRowById_(requestRecord.REQUEST_ID) || findRecentMatchingActiveRequestRow_(requestRecord);
  if (existingRow) {
    requestRecord.REQUEST_ID = existingRow.data.REQUEST_ID;
    requestRecord.SUBMITTED_AT = existingRow.data.SUBMITTED_AT || requestRecord.SUBMITTED_AT;
  }
  var rowNumber = upsertQueuedAccessRequestRecord_(requestRecord, existingRow, skipInitAndSort);
...
function processQueuedIncidentJob_(rawRecord) {
  ...
  // Resolve full name in background — no 3-second limit here
  var submitterId = normalizeText_(incidentRecord._SUBMITTER_ID);
  if (submitterId) {
    try {
      var fullName = resolveSubmitterFullName_({ id: submitterId, name: incidentRecord.SLACK_USER });
      if (fullName) {
        incidentRecord.SLACK_USER = fullName;
      }
    } catch (err) {
      Logger.log('Incident name resolution skipped: ' + err.message);
    }
  }
...
function upsertQueuedAccessRequestRecord_(requestRecord, existingRow, skipInitAndSort) {
  if (existingRow) {
    updateQueuedRequestRecord_(existingRow.rowNumber, requestRecord, existingRow);
    return existingRow.rowNumber;
  }
  return appendAccessRequestRecord_(requestRecord, skipInitAndSort);
}
...
function findRecentMatchingActiveRequestRow_(requestRecord) {
  var rows = getRequestRows_();
  var requestSignature = requestContentSignatureKey_(requestRecord);
  if (!requestSignature) {
    return null;
  }

  var submittedAt = requestRecord.SUBMITTED_AT instanceof Date && !isNaN(requestRecord.SUBMITTED_AT)
    ? requestRecord.SUBMITTED_AT.getTime()
    : null;

  for (var i = 0; i < rows.length; i++) {
    var row = rows[i];
    if (requestContentSignatureKey_(row.data) !== requestSignature) {
      continue;
    }

    var existingSubmitted = row.data.SUBMITTED_AT instanceof Date && !isNaN(row.data.SUBMITTED_AT)
      ? row.data.SUBMITTED_AT.getTime()
      : null;
    if (submittedAt !== null && existingSubmitted !== null && Math.abs(submittedAt - existingSubmitted) > (15 * 60 * 1000)) {
      continue;
    }
    return row;
  }
