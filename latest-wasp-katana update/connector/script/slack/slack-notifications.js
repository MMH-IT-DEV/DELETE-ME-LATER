...
function protectReviewCheckDateColumn_() {
  try {
    var sheet = SpreadsheetApp.getActive().getSheetByName(GMP_SCHEMA.tabs.REVIEW_CHECKS);
    if (!sheet) { return; }

    var dateColumn = GMP_SCHEMA.reviews.columns.NEXT_REVIEW_DUE;
    var range = sheet.getRange(1, dateColumn, sheet.getMaxRows(), 1);

    // Remove any existing protection on this column first
    var existing = sheet.getProtections(SpreadsheetApp.ProtectionType.RANGE);
    for (var i = 0; i < existing.length; i++) {
      if (existing[i].getDescription() === 'Review Check Date - IT Only') {
        existing[i].remove();
      }
    }

    var protection = range.protect().setDescription('Review Check Date - IT Only');
    protection.addEditor('erik@mymagichealer.com');
    ...
    Logger.log('Review date column protection failed: ' + err.message);
  }
}
  ...
  try {
    ...
    UrlFetchApp.fetch(url, {
      ...
      payload: JSON.stringify({
        system: '2026 Security & System Connection Tracker',
        status: 'success',
...
function testSlackConnection() {
  var response = postSlackMessageWithFallback_(
    ...
    'System security tracker connection test',
    [
      buildSectionBlock_('*System security tracker connection test*'),
...
function sendOpenReviewSummary() {
  ...
  var blocks = [
    buildSectionBlock_('*Open system review queue summary*'),
  ...
  postSlackMessageWithFallback_(
    getConfiguredAccessReviewChannels_(),
    'Open system review queue summary',
    blocks
  );
}
...
function onEditInstallable(e) {
  ...
  if (sheetName !== GMP_SCHEMA.tabs.ACCESS_CONTROL &&
      sheetName !== GMP_SCHEMA.tabs.REVIEW_CHECKS &&
      sheetName !== GMP_SCHEMA.tabs.INCIDENTS) {
    return;
  }

  // Skip ensureThreeTabWorkbookReady_() on single-cell edits — it runs 12+
  // normalization passes causing the sheet to blink. The full normalization
  // runs on menu actions and daily maintenance instead.
  try {
    getAccessControlContext_();
  } catch (err) {
    Logger.log('Sheet context not ready for onEdit: ' + err.message);
    return;
  }
  ...
  if (sheetName === GMP_SCHEMA.tabs.REVIEW_CHECKS) {
    handleReviewCheckEdit_(rowNumber, columnNumber, e);
    return;
  }

  if (sheetName === GMP_SCHEMA.tabs.INCIDENTS) {
...
function handleAccessControlEdit_(rowNumber, columnNumber, e) {
  ...
  if (rowNumber > ctx.accountHeaderRow && (ctx.archiveSectionRow === -1 || rowNumber < ctx.archiveSectionRow)) {
    handleAccountEdit_(rowNumber, columnNumber);
    return;
  }
...
function handleAccountEdit_(rowNumber, columnNumber) {
  if (columnNumber !== GMP_SCHEMA.accounts.columns.CURRENT_STATE) {
    return;
  }
  ...
  for (var i = 0; i < accountRows.length; i++) {
    if (accountRows[i].rowNumber === rowNumber) {
      accountRow = accountRows[i];
      break;
    }
  }
  if (!accountRow) {
  ...
  var state = normalizePresenceState_(accountRow.data.CURRENT_STATE);
  if (state === 'Setting Up' || state === 'Revoked') {
    return;
  }

  // Check if there's already an open review for this account
  var reviewRows = getReviewRows_();
  var accountIdentity = accountReusableReviewIdentityKey_(accountRow.data);
  for (var j = 0; j < reviewRows.length; j++) {
    var reviewIdentity = reviewReusableIdentityKey_(reviewRows[j].data);
    if (reviewIdentity === accountIdentity && normalizeText_(reviewRows[j].data.DECISION_STATUS) !== 'Done') {
      return; // already has an open review
    }
  }
  ...
  try {
    ...
    review.REVIEW_PRIORITY = isPrivilegedAccessLevel_(accountRow.data.ACCESS_LEVEL) ? 'High' : 'Medium';
    review.PERSON = accountRow.data.PERSON;
    review.SYSTEM = accountRow.data.GMP_SYSTEM;
    review.ACCESS_LEVEL = accountRow.data.ACCESS_LEVEL;
    review.PRESENCE_STATUS = state;
    review.WHY_FLAGGED = 'Account status changed to ' + state;
    review.DECISION_STATUS = 'Open';
    review.TRIGGER_TYPE = 'Status Change';
    review.ACCESS_ID = accountRow.data.ACCESS_ID;
    review.COMPANY_EMAIL = accountRow.data.COMPANY_EMAIL;
    review.LINKED_REQUEST = accountRow.data.SOURCE_REQUEST_ID;
    review.NEXT_REVIEW_DUE = '';
    review.NEXT_SLACK_TRIGGER = 'Post review queue alert';
    appendReviewRecord_(review, true);
  } catch (err) {
    Logger.log('Auto-review on account status change failed: ' + err.message);
  }
}
...
function handleRequestEdit_(requestRows, rowNumber, columnNumber) {
  ...
  var requestRow = null;
  for (var i = 0; i < requestRows.length; i++) {
    if (requestRows[i].rowNumber === rowNumber) {
  ...
  var approval = normalizeText_(requestRow.data.APPROVAL);
  if (approval !== 'Approved' && approval !== 'Denied') {
    return;
  }

  try {
    var result = syncAccessRequestRowToAccount_(requestRow, true);
    if (result && result.archiveRequest) {
      // Archive immediately while data is fresh and complete
      archiveActiveRequestById_(requestRow.data.REQUEST_ID);
    }
  } catch (err) {
    Logger.log('Auto-sync on edit failed for row ' + rowNumber + ': ' + err.message);
  }
}
...
function handleReviewCheckEdit_(rowNumber, columnNumber, e) {
  ...
  if (normalizeText_(reviewRow.data.DECISION_STATUS) === 'Done' &&
    ...
    refreshReviewWorkflowViews_({ sortAccounts: false });
    return;
  }
  ...
  if (normalizeText_(reviewRow.data.DECISION_STATUS) === 'Done' &&
    ...
    reviewRow.data.DECISION_STATUS = 'Open';
    updateReviewRow_(reviewRow.rowNumber, reviewRow.data);
    showSheetToast_('Need Info — stays open', 'Set Reviewer Action to Keep, Reduce, or Remove once you have the info.', 5);
    return;
  }

  applyReviewDecisionToAccount_(reviewRow);
  refreshReviewWorkflowViews_({ sortAccounts: true });
...
function refreshReviewWorkflowViews_(options) {
  // Skip heavy sort/rewrite on single-cell edits to prevent sheet blinking.
  // Reviews are written in-place immediately; visual reorganization into
  // Open/Scheduled/Completed sections happens on next menu action or daily trigger.
}

function buildAccountRowIndexForReviews_() {
  var rows = getAccountRows_();
  var byAccessId = {};
  var byIdentity = {};

  for (var i = 0; i < rows.length; i++) {
    var accessId = normalizeIdentifier_(rows[i].data.ACCESS_ID);
    if (accessId && !byAccessId[accessId]) {
      byAccessId[accessId] = rows[i];
    }

    var identityKey = accountReusableReviewIdentityKey_(rows[i].data);
    if (identityKey && !byIdentity[identityKey]) {
      byIdentity[identityKey] = rows[i];
    }
  }

  return {
    byAccessId: byAccessId,
    byIdentity: byIdentity
  };
}

function findIndexedAccountRowForReview_(review, accountIndex) {
  if (!accountIndex) {
    return findAccountRowForReview_(review);
  }

  var accessId = normalizeIdentifier_(review.ACCESS_ID);
  if (accessId && accountIndex.byAccessId[accessId]) {
    return accountIndex.byAccessId[accessId];
  }

  var identityKey = reviewReusableIdentityKey_(review);
  if (identityKey && accountIndex.byIdentity[identityKey]) {
    return accountIndex.byIdentity[identityKey];
  }
...
function handleBulkReviewCheckDateEdit_() {
  ...
  var reviewRows = getReviewRows_();
  var accountIndex = buildAccountRowIndexForReviews_();
  var updatedCount = 0;
  for (var i = 0; i < reviewRows.length; i++) {
    var review = reviewRows[i].data;
    var isDone = normalizeText_(review.DECISION_STATUS) === 'Done';

    // Open rows that already have a date — preserve their existing schedule
    if (!isDone && review.NEXT_REVIEW_DUE instanceof Date && !isNaN(review.NEXT_REVIEW_DUE)) {
      continue;
    }
    ...
    review.NOTES = appendUniqueNoteLine_(
      review.NOTES,
      formatDateOnly_(today) + ': ' + (isDone ? 'next cycle set ' : 'bulk follow-up set ') + formatDateOnly_(checkDate)
    );
    updateReviewRow_(reviewRows[i].rowNumber, review);

    var accountRow = findIndexedAccountRowForReview_(review, accountIndex);
    if (accountRow) {
...
function handleReviewCheckDateEdit_(rowNumber) {
  ...
  if (!checkDate) {
    ...
    refreshReviewWorkflowViews_({ sortAccounts: !!account });
    return;
  }
...
function applyReviewDecisionToAccount_(reviewRow) {
  ...
  // Always set the review timestamp and reviewer — even if account is not found
  if (normalizeText_(review.DECISION_STATUS) === 'Done') {
    review.DECISION_DATE = review.DECISION_DATE || new Date();
    try {
      var activeUser = Session.getActiveUser().getEmail();
      if (activeUser) { review.REVIEWED_BY = activeUser; }
    } catch (err) {}
    updateReviewRow_(reviewRow.rowNumber, review);
  }
  ...
  if (normalizeText_(review.REVIEWER_ACTION) === 'Keep') {
    ...
    account.REVIEW_STATUS = 'Done';
    account.NEXT_REVIEW_DUE = nextReviewDueFromDecision_(review.DECISION_DATE, account.GMP_SYSTEM, account.ACCESS_LEVEL, review.REVIEWER_ACTION);
    account.NOTES = appendUniqueNoteLine_(account.NOTES, formatDateOnly_(review.DECISION_DATE) + ': review completed Reduce — access set to ' + account.ACCESS_LEVEL);
  ...
  // Show next review date on the completed review row
  if (account.NEXT_REVIEW_DUE) {
