...
function migrateGmpLabelsToSystem_() {
  var ss = SpreadsheetApp.getActive();
  var replacements = [
    ['ACTIVE GMP ACCOUNTS',  'ACTIVE SYSTEM ACCOUNTS'],
    ['GMP Access Control',   'System Access Control'],
    ['GMP System',           'System'],
    ['GMP Impact',           'System Impact'],
    ['GMP Security',         'System Security'],
    ['Present',              'Access Granted'],
    ['Account Status',       'Access Status'],
    ['Unknown',              '']
  ];
  for (var i = 0; i < replacements.length; i++) {
    ss.createTextFinder(replacements[i][0])
      .matchEntireCell(true)
      .matchCase(false)
      .replaceAllWith(replacements[i][1]);
  }
}

function ensureThreeTabWorkbookReady_() {
  try {
    migrateGmpLabelsToSystem_();
  } catch (err) {}
  try {
    normalizeAccessControlHeaders_();
  } catch (err) {}
  try {
    repairMisplacedRequestRowsInAccountSection_();
  } catch (err) {}
  try {
    dedupeRecentDuplicateActiveRequestRows_();
  } catch (err) {}
  try {
...
function getThreeTabWorkbookIssues_() {
  ...
  var accessCtx = getAccessControlContext_();

  if (!sheetHeaderMatches_(accessSheet, accessCtx.requestHeaderRow, GMP_SCHEMA.requests.headers) ||
      !sheetHeaderMatches_(accessSheet, accessCtx.accountHeaderRow, GMP_SCHEMA.accounts.headers)) {
...
function headerMatchesExpected_(actualValue, expectedHeader) {
  ...
  if (expectedHeader === 'System' && actual === 'GMP System') {
    return true;
  }
  if (expectedHeader === 'System Impact' && actual === 'GMP Impact') {
    return true;
  }
  if (expectedHeader === 'Access Setup Status' && actual === 'Provisioning') {
    return true;
  }
  if (expectedHeader === 'Access Status' && (actual === 'Presence Status' || actual === 'Current State')) {
    return true;
  }
...
function repairMisplacedRequestRowsInAccountSection_() {
  var ctx = ensureArchivedRequestsSection_();
  var startRow = ctx.accountHeaderRow + 1;
  var endRow = ctx.archiveSectionRow - 1;
  if (endRow < startRow) {
    return;
  }

  var width = Math.max(GMP_SCHEMA.requests.keys.length, GMP_SCHEMA.accounts.keys.length);
  var values = ctx.sheet.getRange(startRow, 1, endRow - startRow + 1, width).getValues();
  var rowsToDelete = [];
  var misplacedActiveRequests = [];
  var misplacedArchivedRequests = [];

  for (var i = 0; i < values.length; i++) {
    var rowValues = values[i];
    if (isBlankRow_(rowValues) || isDisplayGroupRow_(rowValues)) {
      continue;
    }

    if (isMisplacedRequestHeaderRow_(rowValues)) {
      rowsToDelete.push(startRow + i);
      continue;
    }

    if (!looksLikeMisplacedRequestRecordRow_(rowValues)) {
      continue;
    }

    var requestRecord = rowToObject_(rowValues, GMP_SCHEMA.requests.keys, GMP_SCHEMA.requests.columns);
    requestRecord.GMP_SYSTEM = canonicalSystemName_(requestRecord.GMP_SYSTEM);
    requestRecord.PROVISIONING = normalizeSetupStatus_(requestRecord.PROVISIONING);
    if (normalizeText_(requestRecord.APPROVAL) === 'Submitted') {
      misplacedActiveRequests.push(requestRecord);
    } else {
      misplacedArchivedRequests.push(requestRecord);
    }
    rowsToDelete.push(startRow + i);
  }
  ...
  if (rowsToDelete.length) {
    ...
    for (var j = 0; j < rowsToDelete.length; j++) {
      ctx.sheet.deleteRow(rowsToDelete[j]);
    }
  }

  for (var k = 0; k < misplacedActiveRequests.length; k++) {
    upsertActiveRequestRecord_(misplacedActiveRequests[k]);
  }
  for (var m = 0; m < misplacedArchivedRequests.length; m++) {
    upsertArchivedRequestRecord_(misplacedArchivedRequests[m]);
  }
}

function isMisplacedRequestHeaderRow_(rowValues) {
  return normalizeText_(rowValues[0]) === 'Request ID' &&
    headerMatchesExpected_(rowValues[1], 'Submitted At') &&
    headerMatchesExpected_(rowValues[2], 'Queue Priority');
}

function looksLikeMisplacedRequestRecordRow_(rowValues) {
  return normalizeText_(rowValues[0]).indexOf('REQ-') === 0;
}

function reconcileRequestSections_() {
  dedupeRequestRowsByIdInSection_(getArchivedRequestRows_());
  purgeArchivedRequestsFromActiveSection_();
  dedupeRequestRowsByIdInSection_(getRequestRows_());
}

function dedupeRecentDuplicateActiveRequestRows_() {
  ...
  var keys = [];
  for (var i = 0; i < rows.length; i++) {
    var signature = requestContentSignatureKey_(rows[i].data);
    if (!signature) {
      continue;
    }
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    if (!requestRowsAreNearDuplicates_(duplicates)) {
      continue;
    }

    var keeper = chooseRequestRowToKeep_(duplicates);
    var merged = mergeDuplicateRequestRows_(duplicates, keeper);
    updateRequestRow_(keeper.rowNumber, merged, keeper.sheetName);
...
function requestContentSignatureKey_(request) {
  var requestType = normalizeText_(request.REQUEST_TYPE).toLowerCase();
  var slackUser = normalizeText_(request.SLACK_USER).toLowerCase();
  var targetUser = normalizeText_(request.TARGET_USER).toLowerCase();
  var companyEmail = normalizeText_(request.COMPANY_EMAIL).toLowerCase();
  var dept = normalizeText_(request.DEPT).toLowerCase();
  var systemName = canonicalSystemName_(request.GMP_SYSTEM).toLowerCase();
  var accessLevel = normalizeText_(request.ACCESS_LEVEL).toLowerCase();
  var gmpImpact = normalizeText_(request.GMP_IMPACT).toLowerCase();
  var reason = normalizeText_(request.REASON).toLowerCase();
  var manager = normalizeText_(request.MANAGER).toLowerCase();
  if (!requestType || !targetUser || !companyEmail || !systemName || !accessLevel) {
    return '';
  }
  return [
    requestType,
...
function requestRowsAreNearDuplicates_(rows) {
  ...
  var maxTime = null;
  for (var i = 0; i < rows.length; i++) {
    var submittedAt = rows[i].data.SUBMITTED_AT;
    if (!(submittedAt instanceof Date) || isNaN(submittedAt)) {
      return false;
    }
    var timeValue = submittedAt.getTime();
    minTime = minTime === null ? timeValue : Math.min(minTime, timeValue);
    maxTime = maxTime === null ? timeValue : Math.max(maxTime, timeValue);
  }

  return (maxTime - minTime) <= (15 * 60 * 1000);
}

function dedupeRequestRowsByIdInSection_(rows) {
  if (!rows || !rows.length) {
    return;
  }
  ...
  for (var i = 0; i < rows.length; i++) {
    var requestId = normalizeText_(rows[i].data.REQUEST_ID);
    if (!requestId) {
      continue;
    }
...
function purgeArchivedRequestsFromActiveSection_() {
  var archivedRows = getArchivedRequestRows_();
  ...
  for (var i = 0; i < archivedRows.length; i++) {
    var archivedId = normalizeText_(archivedRows[i].data.REQUEST_ID);
    if (archivedId) {
      archivedIds[archivedId] = true;
    }
  }

  var activeRows = getRequestRows_();
  var rowsToDelete = [];
  for (var j = 0; j < activeRows.length; j++) {
    var activeId = normalizeText_(activeRows[j].data.REQUEST_ID);
    if (activeId && archivedIds[activeId]) {
      rowsToDelete.push(activeRows[j].rowNumber);
    }
  }
...
function chooseRequestRowToKeep_(rows) {
  var bestRow = rows[0];
  var bestScore = requestRowKeepScore_(rows[0]);
  for (var i = 1; i < rows.length; i++) {
    var score = requestRowKeepScore_(rows[i]);
...
function requestRowKeepScore_(row) {
  var request = row.data;
  var score = 0;
  score += countNonBlankFields_(request, GMP_SCHEMA.requests.keys) * 2;
  ...
  var setupStatus = normalizeSetupStatus_(request.PROVISIONING);
  if (setupStatus === 'Closed' || setupStatus === 'Completed') {
    score += 160;
  } else if (setupStatus === 'Removing Access') {
    score += 140;
  } else if (setupStatus === 'In Progress') {
    score += 120;
  } else if (setupStatus === 'Open') {
    score += 100;
  }

  if (normalizeIdentifier_(request.ACCESS_ID)) {
    score += 80;
  }
  if (request.DECISION_DATE instanceof Date && !isNaN(request.DECISION_DATE)) {
    score += 40;
  }
  if (normalizeText_(request.SLACK_THREAD)) {
    score += 20;
  }
  if (normalizeText_(request.LINKED_REVIEW_ID) || normalizeText_(request.LINKED_INCIDENT_ID)) {
    score += 10;
  }
  return score;
}

function mergeDuplicateRequestRows_(rows, keeperRow) {
  var merged = cloneRecord_(keeperRow.data, GMP_SCHEMA.requests.keys);
  for (var i = 0; i < rows.length; i++) {
    var request = rows[i].data;
    merged.REQUEST_ID = chooseNonBlankText_(merged.REQUEST_ID, request.REQUEST_ID);
    merged.SUBMITTED_AT = chooseEarliestDateValue_(merged.SUBMITTED_AT, request.SUBMITTED_AT);
    merged.QUEUE_PRIORITY = choosePreferredRequestPriority_(merged.QUEUE_PRIORITY, request.QUEUE_PRIORITY);
    merged.REQUEST_TYPE = chooseNonBlankText_(merged.REQUEST_TYPE, request.REQUEST_TYPE);
    merged.SLACK_USER = chooseNonBlankText_(merged.SLACK_USER, request.SLACK_USER);
    merged.TARGET_USER = chooseLongerText_(merged.TARGET_USER, request.TARGET_USER);
    merged.COMPANY_EMAIL = chooseNonBlankText_(merged.COMPANY_EMAIL, request.COMPANY_EMAIL);
    merged.DEPT = chooseNonBlankText_(merged.DEPT, request.DEPT);
    merged.GMP_SYSTEM = canonicalSystemName_(chooseNonBlankText_(merged.GMP_SYSTEM, request.GMP_SYSTEM));
    merged.ACCESS_LEVEL = chooseNonBlankText_(merged.ACCESS_LEVEL, request.ACCESS_LEVEL);
    merged.GMP_IMPACT = chooseNonBlankText_(merged.GMP_IMPACT, request.GMP_IMPACT);
    merged.REASON = chooseLongerText_(merged.REASON, request.REASON);
    merged.MANAGER = chooseNonBlankText_(merged.MANAGER, request.MANAGER);
    merged.IT_OWNER = chooseNonBlankText_(merged.IT_OWNER, request.IT_OWNER);
    merged.APPROVAL = choosePreferredRequestApproval_(merged.APPROVAL, request.APPROVAL);
    merged.PROVISIONING = choosePreferredRequestSetupStatus_(merged.PROVISIONING, request.PROVISIONING);
    merged.NEXT_SLACK_TRIGGER = chooseNonBlankText_(merged.NEXT_SLACK_TRIGGER, request.NEXT_SLACK_TRIGGER);
    merged.SLA_DUE = chooseEarliestDateValue_(merged.SLA_DUE, request.SLA_DUE);
    merged.DECISION_DATE = chooseLatestDateValue_(merged.DECISION_DATE, request.DECISION_DATE);
    merged.ACCESS_ID = normalizeIdentifier_(merged.ACCESS_ID) || normalizeIdentifier_(request.ACCESS_ID);
    merged.LINKED_REVIEW_ID = chooseNonBlankText_(merged.LINKED_REVIEW_ID, request.LINKED_REVIEW_ID);
    merged.LINKED_INCIDENT_ID = chooseNonBlankText_(merged.LINKED_INCIDENT_ID, request.LINKED_INCIDENT_ID);
    merged.SLACK_THREAD = chooseNonBlankText_(merged.SLACK_THREAD, request.SLACK_THREAD);
    merged.NOTES = mergeNoteBlocks_(merged.NOTES, request.NOTES);
  }
  return merged;
}

function choosePreferredRequestPriority_(currentPriority, incomingPriority) {
  var current = normalizeText_(currentPriority);
  var incoming = normalizeText_(incomingPriority);
  return priorityRank_(incoming) < priorityRank_(current) ? incoming : current;
}

function choosePreferredRequestApproval_(currentApproval, incomingApproval) {
  var current = normalizeText_(currentApproval);
  var incoming = normalizeText_(incomingApproval);
  return requestApprovalRank_(incoming) < requestApprovalRank_(current) ? incoming : current;
}

function choosePreferredRequestSetupStatus_(currentStatus, incomingStatus) {
  var current = normalizeSetupStatus_(currentStatus);
  var incoming = normalizeSetupStatus_(incomingStatus);
  return requestSetupStatusRank_(incoming) < requestSetupStatusRank_(current) ? incoming : current;
}

function requestApprovalRank_(approval) {
  var value = normalizeText_(approval);
  if (value === 'Approved') {
    return 0;
  }
  if (value === 'Denied') {
...
function requestSetupStatusRank_(status) {
  var value = normalizeSetupStatus_(status);
  if (value === 'Closed' || value === 'Completed') {
    return 0;
  }
  if (value === 'Removing Access') {
    return 1;
  }
...
function countNonBlankFields_(record, keys) {
  var count = 0;
  for (var i = 0; i < keys.length; i++) {
    if (normalizeText_(record[keys[i]]) !== '') {
      count++;
    }
  }
  return count;
...
function getAccessControlContext_() {
  ...
  var requestHeaderRow = requestSectionRow === -1 ? findHeaderRowByHeaders_(sheet, GMP_SCHEMA.requests.headers, 1) : requestSectionRow + 1;
  var accountHeaderRow = accountSectionRow === -1 ? findHeaderRowByHeaders_(sheet, GMP_SCHEMA.accounts.headers, requestHeaderRow + 1) : accountSectionRow + 1;
  var archiveHeaderRow = archiveSectionRow === -1 ? findHeaderRowByHeaders_(sheet, GMP_SCHEMA.requests.headers, accountHeaderRow + 1) : archiveSectionRow + 1;

  if (requestSectionRow === -1 && requestHeaderRow !== -1) {
    requestSectionRow = requestHeaderRow - 1;
  }
  if (accountSectionRow === -1 && accountHeaderRow !== -1) {
    accountSectionRow = accountHeaderRow - 1;
  }
  if (archiveSectionRow === -1 && archiveHeaderRow !== -1) {
    archiveSectionRow = archiveHeaderRow - 1;
  }

  if (requestSectionRow === -1 || accountSectionRow === -1 || requestHeaderRow === -1 || accountHeaderRow === -1) {
    throw new Error('Access Control sheet sections are missing.');
  }
  return {
    sheet: sheet,
    requestSectionRow: requestSectionRow,
    requestHeaderRow: requestHeaderRow,
    accountSectionRow: accountSectionRow,
    accountHeaderRow: accountHeaderRow,
    archiveSectionRow: archiveSectionRow,
    archiveHeaderRow: archiveHeaderRow
  };
}

function findHeaderRowByHeaders_(sheet, headers, startRow) {
  var firstRow = Math.max(startRow || 1, 1);
  var lastRow = Math.max(sheet.getLastRow(), firstRow);
  for (var rowNumber = firstRow; rowNumber <= lastRow; rowNumber++) {
    if (sheetHeaderMatches_(sheet, rowNumber, headers)) {
      return rowNumber;
    }
  }
  return -1;
...
function appendAccessRequestRecord_(record, skipInitAndSort) {
  if (!skipInitAndSort) {
...
function upsertActiveRequestRecord_(record) {
  var ctx = getAccessControlContext_();
  var sheet = ctx.sheet;
  var existingRow = findActiveRequestRowById_(record.REQUEST_ID);
  var targetRow = existingRow ? existingRow.rowNumber : ctx.requestHeaderRow + 1;

  if (!existingRow) {
    var firstDataRow = ctx.requestHeaderRow + 1;
...
function appendAccountRecord_(record, skipInitAndSort) {
  if (!skipInitAndSort) {
    ensureThreeTabWorkbookReady_();
  }
  ...
  applyDateFormatsByKeys_(ctx.sheet, GMP_SCHEMA.accounts.columns, ['LAST_VERIFIED_AT'], targetRow, 1, 'yyyy-mm-dd hh:mm');
  applyDateFormatsByKeys_(ctx.sheet, GMP_SCHEMA.accounts.columns, ['NEXT_REVIEW_DUE'], targetRow, 1, 'yyyy-mm-dd');
  if (!skipInitAndSort) {
    sortAccountSection_();
  }
...
function appendReviewRecord_(record, skipInitAndSort) {
  if (!skipInitAndSort) {
    ensureThreeTabWorkbookReady_();
  }
  var sheet = getRequiredSheet_(GMP_SCHEMA.tabs.REVIEW_CHECKS);
...
function updateAccountRow_(rowNumber, record) {
  ...
  applyDateFormatsByKeys_(sheet, GMP_SCHEMA.accounts.columns, ['LAST_VERIFIED_AT'], rowNumber, 1, 'yyyy-mm-dd hh:mm');
  applyDateFormatsByKeys_(sheet, GMP_SCHEMA.accounts.columns, ['NEXT_REVIEW_DUE'], rowNumber, 1, 'yyyy-mm-dd');
}
...
function deleteActiveRequestById_(requestId) {
  var requestRow = findActiveRequestRowById_(requestId);
  if (!requestRow) {
    return false;
  }
  getRequiredSheet_(requestRow.sheetName || GMP_SCHEMA.tabs.ACCESS_CONTROL).deleteRow(requestRow.rowNumber);
  return true;
}
...
function sortAccountSection_() {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    var account = rowToObject_(values[i], GMP_SCHEMA.accounts.keys, GMP_SCHEMA.accounts.columns);
    // Remove revoked accounts from the active section
  ...
  // Pad rows to full sheet width so excess columns from previous wider schema are cleared
  var fullWidth = Math.max(GMP_SCHEMA.accounts.keys.length, GMP_SCHEMA.requests.keys.length, ctx.sheet.getMaxColumns());
  var output = [];
  for (var m = 0; m < renderedRows.length; m++) {
    var row = renderedRows[m].slice();
    while (row.length < fullWidth) { row.push(''); }
    output.push(row);
  }
  while (output.length < requiredRows) {
    output.push(buildBlankRow_(fullWidth));
  }

  var targetRange = ctx.sheet.getRange(startRow, 1, requiredRows, fullWidth);
...
function dedupeReviewRecordsForOutput_(reviews) {
  ...
  for (var m = 0; m < deduped.length; m++) {
    ...
    // Keep both the current (open) review AND the latest done review
    var group = byIdentity[rowIdentity];
    var currentReview = null;
    var latestDone = null;
    for (var n = 0; n < group.length; n++) {
      if (normalizeText_(group[n].DECISION_STATUS) === 'Done') {
        if (!latestDone) { latestDone = group[n]; }
        else { latestDone = chooseReviewRecordToKeep_(latestDone, group[n]); }
      } else {
        if (!currentReview) { currentReview = group[n]; }
        else { currentReview = chooseReviewRecordToKeep_(currentReview, group[n]); }
      }
    }
    if (currentReview) { result.push(currentReview); }
    if (latestDone) { result.push(latestDone); }
    if (!currentReview && !latestDone && group.length) { result.push(group[0]); }
...
function buildRenderedAccountRows_(accounts) {
  ...
  for (var j = 0; j < accounts.length; j++) {
    ...
    if (!grouped.hasOwnProperty(label)) {
      label = 'Other Systems';
    }
...
function buildRenderedReviewRows_(reviews) {
  ...
  for (var i = 0; i < reviews.length; i++) {
    ...
    var action = normalizeText_(reviews[i].REVIEWER_ACTION);

    if (status === 'Done' && action === 'Remove') {
      pendingRemovalRows.push(reviews[i]);
    } else if (status === 'Done') {
      completedRows.push(reviews[i]);
    } else if (isScheduledReviewRow_(reviews[i], todayStart)) {
      scheduledRows.push(reviews[i]);
    } else {
  ...
  if (scheduledRows.length) {
    rows = rows.concat(buildRenderedReviewSectionRows_(GMP_SCHEMA.sectionLabels.WAITING_REVIEWS, scheduledRows));
  }

  if (pendingRemovalRows.length) {
    rows = rows.concat(buildRenderedReviewSectionRows_(GMP_SCHEMA.sectionLabels.PENDING_REMOVAL, pendingRemovalRows));
  }
...
function isScheduledReviewRow_(review, todayStart) {
  ...
  // Reviews open 2 days before the due date so the reviewer has prep time
  var openWindow = new Date(todayStart.getTime() + (2 * 86400000));
  ...
  return nextCheckDay.getTime() > openWindow.getTime();
}
...
function formatReviewGroupRows_(sheet, startRow, rowCount) {
  for (var i = 0; i < rowCount; i++) {
    ...
    if (label === GMP_SCHEMA.sectionLabels.OPEN_REVIEWS ||
        ...
        label === GMP_SCHEMA.sectionLabels.PENDING_REMOVAL ||
        label === GMP_SCHEMA.sectionLabels.COMPLETED_REVIEWS) {
      var sectionBg = '#e2e8f0';
      var sectionFg = '#334155';
      if (label === GMP_SCHEMA.sectionLabels.OPEN_REVIEWS) { sectionBg = '#dbeafe'; sectionFg = '#163b65'; }
      else if (label === GMP_SCHEMA.sectionLabels.WAITING_REVIEWS) { sectionBg = '#fef3c7'; sectionFg = '#92400e'; }
      else if (label === GMP_SCHEMA.sectionLabels.PENDING_REMOVAL) { sectionBg = '#fee2e2'; sectionFg = '#991b1b'; }
      rowRange
        .setBackground(sectionBg)
        .setFontColor(sectionFg)
        .setFontWeight('bold')
        .setHorizontalAlignment('left')
        .setVerticalAlignment('middle')
        .clearDataValidations();
...
function accountSystemGroupLabel_(systemName) {
  ...
  if (canonical) {
    for (var i = 0; i < ACCOUNT_SYSTEM_GROUPS.length; i++) {
      if (ACCOUNT_SYSTEM_GROUPS[i] === canonical) {
        return canonical;
      }
    }
  }
