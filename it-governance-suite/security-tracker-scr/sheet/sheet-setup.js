...
var HEADER_ALIASES = {
  'Account Status': ['Presence Status', 'Current State'],
...
function setupSheet() {
  ...
  var message = 'Three-tab GMP workbook is ready.\n\n' +
    'Sheets:\n' +
...
function accessControlMatchesSchema_(sheet) {
  ...
  return sheetHeaderMatches_(sheet, requestSectionRow + 1, GMP_SCHEMA.requests.headers) &&
    sheetHeaderMatches_(sheet, accountSectionRow + 1, GMP_SCHEMA.accounts.headers);
}
...
function buildIncidentsSkeleton_(sheet) {
  writeTitleRows_(
    ...
    'Incidents',
    'Use this tab for /gmp-issue and linked GMP access follow-up.'
  );
  writeSectionRow_(
    sheet,
...
function formatAccessControlSheet_(sheet) {
  ...
  applyDateFormatsByKeys_(sheet, GMP_SCHEMA.accounts.columns, ['LAST_VERIFIED_AT', 'LAST_LOGIN', 'GRANTED_ON'], ctx.accountHeaderRow + 1, accountRowCount, 'yyyy-mm-dd hh:mm');
  applyDateFormatsByKeys_(sheet, GMP_SCHEMA.accounts.columns, ['NEXT_REVIEW_DUE', 'LAST_REVIEW_DATE'], ctx.accountHeaderRow + 1, accountRowCount, 'yyyy-mm-dd');
...
function formatIncidentsSheet_(sheet) {
  writeTitleRows_(
    ...
    GMP_SCHEMA.incidents.headers.length,
    'Incidents',
    'Report GMP issues from Slack and keep linked status updates in the same thread.'
  );
  writeSectionRow_(
    sheet,
    GMP_SCHEMA.layout.INCIDENT_SECTION_ROW,
...
function applyAccessControlConditionalFormatting_(sheet, ctx) {
  ...
  rules = rules.concat(buildTextRules_(sheet, ctx.accountHeaderRow + 1, 2000, GMP_SCHEMA.accounts.columns.ACTIVITY_BAND, {
    'Verified Active': '#dcfce7',
    'Some Evidence': '#dbeafe',
    'No Evidence': '#fee2e2',
    Unknown: '#fef3c7'
  }));
  rules = rules.concat(buildTextRules_(sheet, ctx.accountHeaderRow + 1, 2000, GMP_SCHEMA.accounts.columns.REVIEW_STATUS, {
    Open: '#fee2e2',
    Waiting: '#fef3c7',
    Done: '#dcfce7'
  }));
...
function applyReviewConditionalFormatting_(sheet) {
  var rules = [];
  rules = rules.concat(buildTextRules_(sheet, GMP_SCHEMA.layout.REVIEW_HEADER_ROW + 1, 2000, GMP_SCHEMA.reviews.columns.REVIEW_PRIORITY, {
    High: '#fee2e2',
    Medium: '#fef3c7',
    Low: '#dbeafe'
  }));
  rules = rules.concat(buildTextRules_(sheet, GMP_SCHEMA.layout.REVIEW_HEADER_ROW + 1, 2000, GMP_SCHEMA.reviews.columns.DECISION_STATUS, {
    Open: '#fee2e2',
    Waiting: '#fef3c7',
    Done: '#dcfce7'
  }));
...
function setAccessControlColumnVisibility_(sheet) {
  var maxColumns = Math.max(sheet.getMaxColumns(), GMP_SCHEMA.accounts.headers.length);
  ...
  sheet.hideColumns(17, Math.max(maxColumns - 16, 0));
}
...
function setReviewChecksColumnVisibility_(sheet) {
  ...
  sheet.hideColumns(14, Math.max(maxColumns - 13, 0));
}
