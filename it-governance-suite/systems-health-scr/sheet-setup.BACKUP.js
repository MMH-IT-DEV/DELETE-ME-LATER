...
var TYPE_ORDER = ['FLOW', 'SCRIPT', 'BOT', 'GOOGLE SHEET'];
var NUM_COLS   = 19;
...
var SYSTEMS = {
  'FLOW': [
    ...
    ['Repeat High-Volume Purchase Detection',
     ...
     'N/A',
     '', '', 'Felippe', 'Pending', '', 'Flow HTTP step (both branches)', 'Unknown', '', '',
    ...
    ['Shopify Hold → ShipStation Hold Sync',
     ...
     '2027-01-01', 304, 'Felippe', 'Pending', '', 'Flow HTTP step (pre-condition)', 'Unknown', '', '',
    ...
    ['Shopify Hold Release → ShipStation Release',
     ...
     'SOP-008-ShipStation-Shopify-Flow', 'Bearer Token',
     'Shopify Secrets → Secrets Manager',
     '2027-01-01', 305, 'Felippe', 'Pending', '', 'Flow HTTP step (pre-condition)', 'Unknown', '', '',
  ...
  'SCRIPT': [
    ...
    ['QA Escalation Automation',
     ...
     '', '', 'Erik', 'Pending', '', 'Script sendHeartbeat()', 'Healthy',
     '2026-01-27', 'Yes', ''],
  ...
  'BOT': [
    ['FedEx Dispute Bot',
     ...
     '', '', 'Erik', 'Pending', '', 'Python health_monitor.py', 'Healthy',
  ...
  'GOOGLE SHEET': [
    ...
    ['2026_Katana-Wasp Inventory Sync',
     ...
     'GOOGLE SHEET', 'Wasp/Katana', 'Yes', 'Hourly (via engin-src)',
     'SOP-WASP-Katana-Sync', 'API Key',
     'Katana Settings → Script Properties',
    ...
    ['2026-Katana-WASP_DebugLog',
     ...
     'GOOGLE SHEET', 'Wasp/Katana', 'Yes', 'Per webhook event',
...
function setupHealthSheet() {
  ...
  for (var ti = 0; ti < TYPE_ORDER.length; ti++) {
    ...
    for (var dr = 0; dr < typeRows.length; dr++) {
      ...
      // Days Left      (col K = 11) — colour if expiring
      var daysLeft = row[10];
      if (typeof daysLeft === 'number' && !isNaN(daysLeft)) {
        var dCell = sheet.getRange(writeRow, 11);
        if (daysLeft < 0) {
  ...
  if (lastRow >= 2) {
    addDropdown_(sheet, 2, 3,  lastRow, ['FLOW', 'SCRIPT', 'BOT', 'GOOGLE SHEET']);
    addDropdown_(sheet, 2, 5,  lastRow, ['Yes', 'No']);
    addDropdown_(sheet, 2, 8,  lastRow, ['API Key', 'Bearer Token', 'OAuth', 'Service Account', 'Apps Script Property', 'None']);
