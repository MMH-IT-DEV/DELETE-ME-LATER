...
function resetActivityQAPreviewSheet_(sheet) {
  ensureSheetGridSize_(sheet, 120, 15);
...
function resetActivityQAReportSheet_(sheet) {
  ensureSheetGridSize_(sheet, 80, 8);
...
function resetActivityQARealPreviewSheet_(sheet) {
  ensureSheetGridSize_(sheet, 160, 15);
  sheet.getRange(1, 1, 3, 15).breakApart();
  sheet.clear();
...
function resetActivityQARealReportSheet_(sheet) {
  ensureSheetGridSize_(sheet, 120, 8);
  sheet.getRange(1, 1, 2, 8).breakApart();
  sheet.clear();
...
function buildActivityConformanceFixtures_() {
  return [
    ...
    },
    {
      ...
      context: 'WASP -> Katana @ PROD-RECEIVING @ MMH Kelowna',
      status: 'success',
      linkInfo: buildActivityFixtureLink_('sa', 22080, 'SA-22080'),
      subItems: [
        { sku: 'UFC-1OZ', qty: 1050, uom: 'each', action: 'PROD-RECEIVING  add', status: 'Synced', qtyColor: 'green' }
      ]
    },
    {
      flow: 'F2',
      ...
      subItems: [
        { sku: 'UFC-1OZ', qty: 1050, uom: 'each', action: 'PROD-RECEIVING  lot not in Katana', status: 'Skipped', qtyColor: 'grey' }
      ]
    },
    {
      flow: 'F2',
      ...
      context: '<- PROD-RECEIVING @ MMH Kelowna',
      status: 'reverted',
      linkInfo: buildActivityFixtureLink_('sa', 22082, 'SA-22082'),
      subItems: [
        { sku: 'UFC-1OZ', qty: 1050, uom: 'each', action: 'PROD-RECEIVING  restore', status: 'Restored', qtyColor: 'green' }
      ]
    },
    {
      flow: 'F2',
      ...
      subItems: [
        { sku: 'UFC-1OZ', qty: 600, uom: 'each', action: 'PROD-RECEIVING  add', isParent: true, batchCount: 2, qtyColor: 'grey' },
    ...
    },
    {
      ...
      context: 'SW-STORAGE -> RECEIVING-DOCK',
      status: 'success',
      linkInfo: buildActivityFixtureLink_('st', 253, 'ST-253'),
      subItems: [
        { sku: 'AGJAR-2', qty: 7600, uom: 'each', action: 'move SW-STORAGE -> RECEIVING-DOCK', status: 'Synced', qtyColor: 'green' },
        { sku: 'LABSSEAL-2', qty: 1360, uom: 'each', action: 'move SW-STORAGE -> RECEIVING-DOCK', status: 'Synced', qtyColor: 'green' },
        { sku: 'B-YELLOW-2', qty: 550, uom: 'each', action: 'move SW-STORAGE -> RECEIVING-DOCK', status: 'Synced', qtyColor: 'green' }
      ]
    },
    {
      flow: 'F3',
      ...
      context: 'mixed-source -> QA-Hold-1',
      status: 'partial',
    ...
    },
    {
      ...
      context: 'RECEIVING-DOCK -> QA-Hold-1',
      status: 'partial',
      linkInfo: buildActivityFixtureLink_('st', 259, 'ST-259'),
      subItems: [
        { sku: 'UFC-1OZ', qty: 600, uom: 'each', action: 'RECEIVING-DOCK -> QA-Hold-1', isParent: true, batchCount: 2, qtyColor: 'grey' },
    ...
    },
    {
      ...
      context: 'QA-Hold-1 -> RECEIVING-DOCK',
      status: 'reverted',
      linkInfo: buildActivityFixtureLink_('st', 260, 'ST-260'),
      subItems: [
        { sku: 'UFC-1OZ', qty: 600, uom: 'each', action: 'move QA-Hold-1 -> RECEIVING-DOCK', status: 'Synced', qtyColor: 'green' }
      ]
    },
    {
      flow: 'F4',
...
function buildRealActivitySubItems_(flow, block, statusKey) {
  ...
  for (var i = 0; i < block.subRows.length; i++) {
    ...
    if (flow === 'F1') {
      ...
      var f2Action = '';
      if (/lot not in katana/i.test(parsed.tail)) f2Action = f2Loc + '  lot not in Katana';
      else if (/not in katana/i.test(parsed.tail)) f2Action = f2Loc + '  not in Katana';
      else if (rowStatus === 'Restored') f2Action = f2Loc + '  restore';
      else if (/remove/i.test(parsed.tail) || rowStatus === 'Removed') f2Action = f2Loc + '  remove';
      else f2Action = f2Loc + '  add';
      item.action = buildActivityActionText_(f2Action, lot, expiry);
      item.qtyColor = (/remove/i.test(f2Action) || rowStatus === 'Removed' || rowStatus === 'Failed') ? 'red' : 'green';
    } else if (flow === 'F3') {
      ...
      if (item.nested) {
        ...
        item.action = f3From + (f3To ? ' -> ' + f3To : '');
      } else {
        item.action = buildActivityActionText_('move ' + f3From + (f3To ? ' -> ' + f3To : ''), lot, expiry);
        item.qtyColor = rowStatus === 'Failed' ? 'red' : 'green';
      }
    } else if (flow === 'F4') {
...
function buildRealActivityContext_(flow, statusKey, subItems, headerText) {
  ...
  if (flow === 'F2') {
    var f2Loc = findPreviewLocationByStatus_(subItems, '') || extractPreviewLocations_(headerText)[0] || '';
    return statusKey === 'reverted'
      ? ('<- ' + getActivityDisplayLocation_(f2Loc) + (site ? ' @ ' + site : ''))
      : ('WASP -> Katana @ ' + getActivityDisplayLocation_(f2Loc) + (site ? ' @ ' + site : ''));
  }
...
function extractPreviewSite_(headerText) {
  ...
  var siteMatch = text.match(/(MMH\s+[A-Za-z]+(?:\s+[A-Za-z]+)*)/);
