...
function handleWaspQuantityAdded(payload) {
  ...
  // Skip F5-owned locations — SHOPIFY is deducted by F5 (ShipStation).
  // Callouts from these locations must not create Katana SAs.
  if (F2_SKIP_LOCATIONS.indexOf(location) >= 0) {
  ...
  if (wasRecentlySyncedToWasp(sku, location, 'add')) {
...
function handleWaspQuantityRemoved(payload) {
  ...
  // Skip F5-owned locations — SHOPIFY removals are done by F5 (ShipStation).
  // Callouts from these locations must not create Katana SAs (prevents double-deduction).
  if (F2_SKIP_LOCATIONS.indexOf(location) >= 0) {
    return { status: 'skipped', reason: 'Location ' + location + ' managed by F5' };
  }
  ...
  // Check if recently synced — covers both Katana→WASP loops and engin sheet pre-marks
  if (wasRecentlySyncedToWasp(sku, location, 'remove')) {
    return { status: 'skipped', reason: 'Recently synced by Katana handler or engin sheet' };
  }
...
function processBatchIfReady(action, batchId) {
  ...
  // Acquire script lock — blocks other concurrent executions until released.
  // Each doPost invocation is a separate GAS execution so this is safe to call.
  ...
  try {
    ...
    // Mark processed BEFORE calling process functions — prevents any re-entry
    ...
    if (result.success && result.saId) {
      PropertiesService.getScriptProperties().setProperty(
        ...
        JSON.stringify({
          ...
          qty: item.quantity,
          uom: itemUom,
          location: item.location || siteName,
          site: siteName,
          lot: lot,
          expiry: expiry || ''
        })
...
function handleEnginMark(payload) {
  ...
  markSyncedToWasp(sku, location, op);
  ...
  try {
    PropertiesService.getScriptProperties().setProperty('DEBUG_LAST_PREMARK', JSON.stringify({
      t: new Date().toISOString(), sku: sku, loc: location, op: op,
      k1: 'wasp_sync_' + op + '_' + sku + '_' + location,
      k2: 'wasp_sync_' + op + '_' + sku
    }));
  } catch(e) {}
  return { status: 'ok', sku: sku, location: location, op: op };
}

function markSyncedToWasp(sku, location, action) {
  var cache = CacheService.getScriptCache();
...
function wasRecentlySyncedToWasp(sku, location, action) {
  var cache = CacheService.getScriptCache();
  var key    = 'wasp_sync_' + action + '_' + sku + '_' + location;
  var anyKey = 'wasp_sync_' + action + '_' + sku;
  return cache.get(key) !== null || cache.get(anyKey) !== null;
}
